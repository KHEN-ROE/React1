const GalC1 = ({c1, selc1, setSelc1}) => {
    ...
    const Tag1 = c1.map((item) => <div className={item===selc1? "Tag11":"Tag1"} key={item} onClick={()=>handleSelect(item)}>{item}</div>)//title들을 클릭했을 때 변화를 주겠다. GalC1이 구현되어 있는 컴포넌트는 GalMain이다. 따라서 GalMain에 useState를 선언한다.
                                                                                                            //타이틀들을 클릭했을 때 함수 호출
    return(
        <div className="box1">
            {Tag1}
